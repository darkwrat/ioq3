...
void SV_ShutdownGameProgs( void ) {
	...
	VM_SafeCall( gvm, GAME_SHUTDOWN, 1, qfalse );
	VM_Free( gvm );
	gvm = NULL;
}
...
static void SV_InitGameVM( qboolean restart ) {
	...
	VM_SafeCall (gvm, GAME_INIT, 3, sv.time, Com_Milliseconds(), restart);
}
...
void SV_RestartGameProgs( void ) {
	...
	VM_SafeCall( gvm, GAME_SHUTDOWN, 1, qtrue );
...
qboolean SV_GameCommand( void ) {
	...
	return VM_SafeCall( gvm, GAME_CONSOLE_COMMAND, 0);
}
