...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	for (i = 0;i < 3; i++)
	{
		VM_SafeCall (gvm, GAME_RUN_FRAME, 1, sv.time);
		SV_BotFrame (sv.time);
		sv.time += 100;
		svs.time += 100;
	}
	...
	for (i=0 ; i<sv_maxclients->integer ; i++) {
		...
		if (svs.clients[i].state >= CS_CONNECTED) {
			...
			denied = VM_ExplicitArgPtr( gvm, VM_SafeCall( gvm, GAME_CLIENT_CONNECT, 3, i, qfalse, isBot ) );	// firstTime = qfalse
			if ( denied ) {
				...
				if( !isBot ) {
					...
					VM_SafeCall( gvm, GAME_CLIENT_BEGIN, 1, i );
				}
			}
		}
	}	
