...
//			VM_SafeCall( gvm, GAME_CLIENT_DISCONNECT, 1, newcl - svs.clients );
			//
			goto gotnewcl;
		}
	}
...
gotnewcl:	
	...
	*newcl = temp;
	...
	denied = VM_SafeCall( gvm, GAME_CLIENT_CONNECT, 3, clientNum, qtrue, qfalse ); // firstTime = qtrue
	if ( denied ) {
		// we can't just use VM_ArgPtr, because that is only valid inside a VM_SafeCall
...
void SV_DropClient( client_t *drop, const char *reason ) {
	...
	VM_SafeCall( gvm, GAME_CLIENT_DISCONNECT, 1, drop - svs.clients );
...
void SV_ClientEnterWorld( client_t *client, usercmd_t *cmd ) {
	...
	VM_SafeCall( gvm, GAME_CLIENT_BEGIN, 1, client - svs.clients );
}
...
static void SV_UpdateUserinfo_f( client_t *cl ) {
	...
	VM_SafeCall( gvm, GAME_CLIENT_USERINFO_CHANGED, 1, cl - svs.clients );
}
...
void SV_ExecuteClientCommand( client_t *cl, const char *s, qboolean clientOK ) {
	...
	if (clientOK) {
		...
		if (!u->name && sv.state == SS_GAME && (cl->state == CS_ACTIVE || cl->state == CS_PRIMED)) {
			Cmd_Args_Sanitize();
			VM_SafeCall( gvm, GAME_CLIENT_COMMAND, 1, cl - svs.clients );
		}
	}
	else if (!bProcessed)
...
void SV_ClientThink (client_t *cl, usercmd_t *cmd) {
	...
	VM_SafeCall( gvm, GAME_CLIENT_THINK, 1, cl - svs.clients );
}
