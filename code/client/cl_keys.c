...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if ( key == K_ESCAPE ) {
		...
		if (Key_GetCatcher( ) & KEYCATCH_CGAME) {
			...
			VM_SafeCall (cgvm, CG_EVENT_HANDLING, 1, CGAME_EVENT_NONE);
			return;
		}

		if ( !( Key_GetCatcher( ) & KEYCATCH_UI ) ) {
			if ( clc.state == CA_ACTIVE && !clc.demoplaying ) {
				VM_SafeCall( uivm, UI_SET_ACTIVE_MENU, 1, UIMENU_INGAME );
			}
			else if ( clc.state != CA_DISCONNECTED ) {
				CL_Disconnect_f();
				S_StopAllSounds();
				VM_SafeCall( uivm, UI_SET_ACTIVE_MENU, 1, UIMENU_MAIN );
			}
			return;
		}

		VM_SafeCall( uivm, UI_KEY_EVENT, 2, key, qtrue );
		return;
	}
	...
	if ( Key_GetCatcher( ) & KEYCATCH_CONSOLE ) {
		...
		if ( cgvm ) {
			VM_SafeCall( cgvm, CG_KEY_EVENT, 2, key, qtrue );
		} 
...
void CL_KeyUpEvent( int key, unsigned time )
{
	...
	if ( Key_GetCatcher( ) & KEYCATCH_UI && uivm ) {
		VM_SafeCall( uivm, UI_KEY_EVENT, 2, key, qfalse );
	} else if ( Key_GetCatcher( ) & KEYCATCH_CGAME && cgvm ) {
		VM_SafeCall( cgvm, CG_KEY_EVENT, 2, key, qfalse );
	}
}
...
void CL_CharEvent( int key ) {
	...
	{
		VM_SafeCall( uivm, UI_KEY_EVENT, 2, key | K_CHAR_FLAG, qtrue );
	}
	else if ( Key_GetCatcher( ) & KEYCATCH_MESSAGE ) 
	{
