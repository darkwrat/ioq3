...
void CL_ShutdownUI( void ) {
	...
	VM_SafeCall( uivm, UI_SHUTDOWN, 0 );
	VM_Free( uivm );
	uivm = NULL;
}
...
void CL_InitUI( void ) {
	...
	// sanity check
	v = VM_SafeCall( uivm, UI_GETAPIVERSION, 0 );
	if (v == UI_OLD_API_VERSION) {
//		Com_Printf(S_COLOR_YELLOW "WARNING: loading old Quake III Arena User Interface version %d\n", v );
		// init for this gamestate
		VM_SafeCall( uivm, UI_INIT, 1, (clc.state >= CA_AUTHORIZING && clc.state < CA_ACTIVE));
	}
...
qboolean UI_usesUniqueCDKey( void ) {
	if (uivm) {
		return (VM_SafeCall( uivm, UI_HASUNIQUECDKEY, 0 ) == qtrue);
	} else {
		return qfalse;
	}
}
...
qboolean UI_GameCommand( void ) {
	...
	return VM_SafeCall( uivm, UI_CONSOLE_COMMAND, 1, cls.realtime );
}
