...
void CL_ShutdownCGame( void ) {
	...
	VM_SafeCall( cgvm, CG_SHUTDOWN, 0 );
	VM_Free( cgvm );
	cgvm = NULL;
}
...
void CL_InitCGame( void ) {
	...
	VM_SafeCall( cgvm, CG_INIT, 3, clc.serverMessageSequence, clc.lastExecutedServerCommand, clc.clientNum );
...
qboolean CL_GameCommand( void ) {
	...
	return VM_SafeCall( cgvm, CG_CONSOLE_COMMAND, 0 );
}
...
void CL_CGameRendering( stereoFrame_t stereo ) {
	VM_SafeCall( cgvm, CG_DRAW_ACTIVE_FRAME, 3, cl.serverTime, stereo, clc.demoplaying );
	VM_Debug( 0 );
}
