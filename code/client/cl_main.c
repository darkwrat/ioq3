	...
	while(target)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					val = VM_SafeCall(cgvm, CG_LAST_ATTACKER, 0);
					target += 8;
				}
				else if(!Q_stricmpn(target, "crosshair", 9))
				{
					val = VM_SafeCall(cgvm, CG_CROSSHAIR_PLAYER, 0);
					target += 9;
				}
				else
				{
...
void CL_Disconnect( qboolean showMainMenu ) {
	...
	*clc.downloadTempName = *clc.downloadName = 0;
	...
	if ( uivm && showMainMenu ) {
		VM_SafeCall( uivm, UI_SET_ACTIVE_MENU, 1, UIMENU_NONE );
	}
...
void CL_Frame ( int msec ) {
	...
	if ( cls.cddialog ) {
		...
		cls.cddialog = qfalse;
		VM_SafeCall( uivm, UI_SET_ACTIVE_MENU, 1, UIMENU_NEED_CD );
